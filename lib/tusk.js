...
function document(parts,done){
	step(
		function work(){
			...
			documenters.examples(parts.input,this.parallel());
		...
		function finalize(err,showcase,readme,licence,lib,examples,style,script,dotTusk,githubButtons,badges){
			if(err) throw err;
			var content = [readme,examples,lib,licence].reduce(function(content,section){
